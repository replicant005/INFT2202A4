 ...
 */
export const renderOpportunitiesPage = async (): Promise<string> => {
    try {
 ...
 */
export const initOpportunitiesPage = async (): Promise<void> => {
    try {
 ...
 */
const getOpportunities = async (): Promise<VolunteerOpportunity[]> => {
 ...
 */
const displayOpportunities = (opportunities: VolunteerOpportunity[]): void => {
    const opportunityCards = document.getElementById('opportunity-cards');
 ...
 * @param e Click event
 */
const handleSignupClick = (e: Event): void => {
    ...
    if (opportunityIndex) {
        ...
        if (modalTitle) {
            const opportunityTitle = button.closest('.card')?.querySelector('.card-title')?.textContent;
 ...
 */
const setupSignupFormHandler = (): void => {
    const signupForm = document.getElementById('signupForm') as HTMLFormElement;
...
const validateSignupForm = (): boolean => {
    ...
    if (!roleInput.value.trim()) {
        ...
        if (roleError) roleError.textContent = 'Preferred role is required';
        isValid = false;
    }
