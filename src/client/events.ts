 ...
 */
export const renderEventsPage = async (): Promise<string> => {
    try {
 ...
 */
export const initEventsPage = async (): Promise<void> => {
    try {
        // Fetch events data
 ...
 */
const displayEvents = (events: Event[]): void => {
    const eventList = document.getElementById('event-list');
 ...
 */
const attachFilterEventListeners = (): void => {
 ...
 */
const filterEvents = (category: string): void => {
    const eventCards = document.querySelectorAll('.event-card');
    
    eventCards.forEach(card => {
        const cardCategory = card.getAttribute('data-category');
        const container = (card as HTMLElement).closest('.col-md-6');
